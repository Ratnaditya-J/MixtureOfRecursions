...
class LearnedThresholdRouter(nn.Module):
    ...
    def forward(
        ...
        thresholds = torch.clamp(
            thresholds, self.config.min_threshold, self.config.max_threshold
        )
        ...
        if training:
            ...
            gumbel_noise = -torch.log(
                -torch.log(torch.rand_like(depth_probs) + 1e-8) + 1e-8
            )
            depth_logits = torch.log(depth_probs + 1e-8) + gumbel_noise
            depth_probs_soft = F.softmax(depth_logits / 0.5, dim=-1)
            recursion_depths = torch.sum(
                depth_probs_soft
                * torch.arange(
                    1, self.config.max_recursion_depth + 1, device=depth_probs.device
                ),
                dim=-1,
            )
        else:
            # Deterministic assignment
            recursion_depths = torch.sum(depth_probs, dim=-1).clamp(
                1, self.config.max_recursion_depth
            )
...
class MultiScaleAttention(nn.Module):
    ...
    def __init__(self, config: AdvancedMoRConfig):
        ...
        self.scale_fusion = nn.Linear(
            len(self.scales) * self.hidden_size, self.hidden_size
        )
    ...
    def forward(
        ...
        for scale_idx, (scale, attention) in enumerate(
            zip(self.scales, self.scale_attentions)
        ):
            if scale == 1:
                # Standard attention
                ...
                if downsampled_len > 0:
                    ...
                    downsampled = F.avg_pool1d(
                        hidden_states.transpose(1, 2), kernel_size=scale, stride=scale
                    ).transpose(1, 2)
...
class AdvancedMixtureOfRecursions(MixtureOfRecursions):
    ...
    def __init__(self, config: AdvancedMoRConfig):
        ...
        base_config = MoRConfig(
            **{k: v for k, v in config.__dict__.items() if hasattr(MoRConfig, k)}
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        recursion_depths, router_logits, aux_outputs = self.router(
            hidden_states, training=self.training, current_efficiency=current_efficiency
        )
    ...
    def _compute_advanced_router_loss(
        ...
        efficiency_loss = 0.0
        if computation_log:
            avg_efficiency = sum(log["efficiency"] for log in computation_log) / len(
                computation_log
            )
            efficiency_target = self.config.target_efficiency
            efficiency_loss = F.mse_loss(
                torch.tensor(avg_efficiency), torch.tensor(efficiency_target)
            )
        ...
        total_loss = (
            base_loss + self.config.efficiency_weight * efficiency_loss + threshold_loss
        )
...
def create_advanced_mor_model(
    model_size: str = "small", use_all_features: bool = True
