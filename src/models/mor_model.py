...
class RecursiveRouter(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, training: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if training:
            ...
            recursion_probs = F.gumbel_softmax(router_logits, tau=self.temperature, hard=True)
            recursion_depths = torch.argmax(recursion_probs, dim=-1) + self.config.min_recursion_depth
        else:
            # Use argmax for inference
            recursion_depths = torch.argmax(router_logits, dim=-1) + self.config.min_recursion_depth
    ...
    def forward(
        ...
        if key_value_cache is not None:
            ...
            attn_output, attn_weights = self.attention(
                hidden_states, cached_key, cached_value,
...
class MixtureOfRecursions(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        recursion_depths, router_logits = self.router(hidden_states, training=self.training)
        ...
        for depth in range(1, self.config.max_recursion_depth + 1):
            ...
            hidden_states, kv_cache = layer(
                ...
                key_value_cache=kv_cache if depth > 1 and self.config.use_kv_sharing else None,
                use_cache=use_cache
            )
    ...
    def _create_attention_mask(self, base_mask: torch.Tensor, depth_mask: torch.Tensor) -> torch.Tensor:
    ...
    def _compute_router_loss(self, router_logits: torch.Tensor, recursion_depths: torch.Tensor) -> torch.Tensor:
        ...
        load_balance_loss = F.kl_div(mean_probs.log(), uniform_target, reduction='batchmean')
