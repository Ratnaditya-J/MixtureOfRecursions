...
class MoRAnalyzer:
        ...
        for text in texts:
            ...
            with torch.no_grad():
                if isinstance(self.model, AdvancedMixtureOfRecursions):
                    ...
                    recursion_depths, router_logits = self.model.router(
            ...
            actual_length = (
                (inputs["input_ids"][0] != self.tokenizer.pad_token_id).sum().item()
            )
                    ...
                    results["memory_usage"].append(
                        {
                            ...
                            "seq_len": seq_len,
                            "memory_mb": (
                                memory_used / (1024 * 1024) if memory_used > 0 else 0
                            ),
                        }
                    )
    ...
    def analyze_token_complexity_correlation(self, texts: List[str]) -> Dict[str, Any]:
        """Analyze correlation between token complexity and recursion depth."""

        results = {"correlations": [], "token_analysis": [], "complexity_patterns": {}}
    ...
    def _compute_text_complexity(
        self, text: str, depths: np.ndarray
    ) -> Dict[str, float]:
    ...
    def generate_analysis_report(
        self, texts: List[str], output_path: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        throughput_analysis = self.benchmark_throughput(
            sequence_lengths=[32, 64, 128], batch_sizes=[1, 4], num_runs=5
        )
        ...
        report = {
            ...
            "summary_statistics": self._compute_summary_statistics(
                recursion_analysis, throughput_analysis
            ),
        }
    ...
    def _compute_summary_statistics(
        self, recursion_analysis: Dict, throughput_analysis: Dict
    ) -> Dict[str, Any]:
        ...
        throughputs = [
            item["throughput"] for item in throughput_analysis["throughput_data"]
        ]
        ...
        summary = {
            "recursion_stats": {
                ...
                "avg_efficiency": (
                    float(np.mean(all_efficiencies)) if all_efficiencies else 0
                ),
                "efficiency_std": (
                    float(np.std(all_efficiencies)) if all_efficiencies else 0
                ),
            },
            "performance_stats": {
...
def create_analyzer(
    model_path: Optional[str] = None, model_type: str = "basic"
) -> MoRAnalyzer:
