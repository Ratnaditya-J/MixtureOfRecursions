...
class MoREvaluator:
    ...
    def evaluate_perplexity(
        self, dataset_name: str, split: str = "test", max_samples: int = 1000
    ):
        ...
        depth_stats = {
            ...
            "depth_distribution": {
                ...
                for i in range(
                    self.config.min_recursion_depth, self.config.max_recursion_depth + 1
                )
            },
        }
    ...
    def evaluate_throughput(self, sequence_lengths: list = [128, 256, 512, 1024]):
        ...
        for seq_len in sequence_lengths:
            ...
            print(
                f"Seq len {seq_len}: {tokens_per_second:.1f} tokens/sec, avg depth: {avg_depth:.2f}"
            )
            ...
            for test_text in test_cases:
                ...
                with torch.no_grad():
                    ...
                    ground_truth = (
                        "positive"
                        if any(
                            word in test_text.lower()
                            for word in ["fantastic", "enjoyed", "great", "amazing"]
                        )
                        else "negative"
                    )
    ...
    def generate_comprehensive_report(self, output_path: str):
        ...
        results = {"model_config": self.config.__dict__, "evaluation_results": {}}
        ...
        print(
            f"Average recursion depth: {perplexity_results['recursion_stats']['mean_depth']:.2f}"
        )
        print(f"Few-shot accuracy: {few_shot_results['few_shot_accuracy']:.3f}")
        print(
            f"Peak throughput: {max(t['tokens_per_second'] for t in throughput_results.values()):.1f} tokens/sec"
        )
