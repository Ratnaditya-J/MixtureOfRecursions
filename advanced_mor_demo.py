...
try:
    from src.models.advanced_mor import create_advanced_mor_model, AdvancedMixtureOfRecursions
...
def demonstrate_advanced_features():
    ...
    print(f"Advanced MoR parameters: {sum(p.numel() for p in advanced_model.parameters()):,}")
    print()
    ...
    with torch.no_grad():
        try:
            ...
            for i, (token, depth) in enumerate(zip(tokens[:10], depths[:10])):  # Show first 10 tokens
                complexity = aux_outputs.get('complexity_scores', [torch.zeros(1)])[0]
                if i < len(complexity):
                    comp_score = complexity[i].item() if hasattr(complexity[i], 'item') else 0
                else:
                    comp_score = 0
            ...
            if 'computation_log' in outputs:
                ...
                for log in outputs['computation_log']:
                    print(f"  Depth {log['depth']}: {log['active_tokens']} active tokens ({log['efficiency']:.1%} efficiency)")
    ...
    try:
        ...
        recursion_analysis = analyzer.analyze_recursion_patterns(test_texts[:3])  # Analyze first 3 texts
    ...
    print("  2. Run evaluation: python run_mor_experiment.py evaluate --model_path results/")
    print("  3. Explore notebooks: jupyter notebook notebooks/")
    print("  4. Analyze patterns: python -c \"from src.analysis import create_analyzer; analyzer = create_analyzer(); print('Analysis ready!')\"")
    print()
